...
import starling.display.BlendMode;
	...
	public class BrandView extends Sprite {
        ...
        //Button used to finish the unlock session.
		...
		public function BrandView(__options:BrandViewOptions, __beverageId:String, __language: uint = 0) {
            ...
            if(FountainFamily.LOCKED_MODE) {
                // Done button
                languagesStrings.length = 0;
                for(j = 0; j < FountainFamily.LOCALE_ISO.length; j ++) languagesStrings.push(StringList.getList(FountainFamily.LOCALE_ISO[j]).getString("unlock/session-finish"));
                ...
                if(!options.isADA) {
                    buttonDone.x = buttonBack.x + 2 * buttonDone.radius + 10;
                    buttonDone.y = buttonBack.y;
                } else {
                    buttonDone.y = buttonBack.y - 2 * buttonDone.radius - 10;
                    buttonDone.x = buttonBack.x;
                }
		...
		private function redrawVisibility():void {
            ...
            if(buttonDone != null) buttonDone.visibility = uiPhase;
		...
		private function onButtonBackTapped(__button:BlobButton):void {
            ...
            if(FountainFamily.LOCKED_MODE) {
                if(BackendModel.UNLOCKED_TIME < GPURoot.TIME_BRAND_HIDE) {
                    FountainFamily.backendModel.lockedPourCompleted();
                } else {
        ...
        private function onButtonDoneTapped(_button:BlobButton) : void {
            FountainFamily.backendModel.lockedPourCompleted();
        }
		...
		override public function dispose():void {
            ...
            if(buttonDone != null) {
                ...
                buttonDone.onTapped.remove(onButtonBackTapped);
                buttonDone.stop();
