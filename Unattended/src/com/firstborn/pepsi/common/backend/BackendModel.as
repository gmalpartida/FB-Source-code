...
//      For the unlock events (Cashless / Unattended), do not change
        public static const BACKEND_EVENT_UNLOCKED_AUTHORIZE 			    :String = "Unattended.OnAuthenticationPending";
        public static const BACKEND_EVENT_UNLOCKED_SUCCESS 				    :String = "Unattended.OnAuthenticationSuccess";
        public static const BACKEND_EVENT_UNLOCKED_ERROR 				    :String = "Unattended.OnAuthenticationFailure";
        public static const BACKEND_COMMAND_UNLOCKED_SESSION_END 			:String = "Unattended.OnEndSession";

//        public static const BACKEND_EVENT_UNLOCKED_AUTHORIZE 			    :String = "OnAuthenticationPending";
//        public static const BACKEND_EVENT_UNLOCKED_SUCCESS 				    :String = "OnAuthenticationSuccess";
//        public static const BACKEND_EVENT_UNLOCKED_ERROR 				    :String = "OnAuthenticationFailure";
//        public static const BACKEND_COMMAND_UNLOCKED_SESSION_END 			:String = "OnEndSession";

        //Events for the backend payment methods fired
        public static const EVENT_UNLOCKED_AUTHORIZE						:String = "BackendModel.onPaymentAuthorize";
        public static const EVENT_UNLOCKED_SUCCESS					        :String = "BackendModel.onPaymentSuccess";
        public static const EVENT_UNLOCKED_ERROR					        :String = "BackendModel.onPaymentError";
        public static const EVENT_UNLOCKED_SESSION_FINISH                   :String = "BackendModel.onPaymentSessionFinish";

        public static var UNLOCKED_TIME : int;
        public static var UNLOCKED_REMAINING_TIME : uint;
        public static var UNLOCKED_ERROR : uint;
        ...
        public static const EVENT_UPDATE_UNLOCK_COUNTDOWN                   :String = "BackendModel.onUpdateCountdown";
        ...
        public static var lightihgHardware : String = "A"                   //String ID name for the hardware used to control the lighting (stripe of leds in the border of the screen
        ...
        //static variables used to control the flow of the unlock of the machine...
        public static var UNLOCK_PROCESSING_STATE : Boolean = false;
        public static var UNLOCK_SUCCESS_STATE : Boolean = false;
        public static var UNLOCK_ERROR_STATE : Boolean = false;

		// Properties
		private var currentIds:String;
		private var numEventListeners:uint;

        private static var countdownInterval : uint;                               //Interval used to update the unlock countdown

        public static var APPLY_DELAY : Boolean = false;
		...
		public static function init(__backendInterface:IBackendInterface):void {
			...
			if (backendInterface.available) {
                ...
                //for  Unattended
                backendInterface.addCallback(BACKEND_EVENT_UNLOCKED_AUTHORIZE,					onUnlockedAuthorize);
                backendInterface.addCallback(BACKEND_EVENT_UNLOCKED_SUCCESS,					onUnlockedSuccess);
                backendInterface.addCallback(BACKEND_EVENT_UNLOCKED_ERROR,						onUnlockedError);
			}
		...
		private static function setLightColor(__red:uint, __green:uint, __blue:uint, __brightness:uint, __transitionMS:uint):void {
			...
			if (__red != lightningColorR || __green != lightningColorG || __blue != lightningColorB || __brightness != lightningColorBrightness) {
                ...
                trace("The color send is: " + __red + "  " + __green + "  " + __blue + "  " + __brightness);
        ...
        private static function onUnlockedAuthorize(...__args) : void {

            var delay : Number = 0;
            if(FountainFamily.attractorController.isInIdleState) {
                if(APPLY_DELAY) {
                    delay = 1800;
                    if(FountainFamily.SHOW_UNLOCK_THANKS_MESSAGE) delay = 3700;
                }
            }

            if(!UNLOCK_PROCESSING_STATE) {
                UNLOCK_PROCESSING_STATE = true;
                UNLOCK_ERROR_STATE = false;
                UNLOCK_SUCCESS_STATE = false;
                setTimeout(function() : void {
                    dispatchEvents(new Event(EVENT_UNLOCKED_AUTHORIZE));
                }, delay);
            }
        }

        private static function onUnlockedSuccess(...__args) : void {
            if(!UNLOCK_SUCCESS_STATE && UNLOCK_PROCESSING_STATE) {
                if(UNLOCKED_TIME == 0) {

                    var delay : Number = 1800;
                    if(FountainFamily.attractorController.isInIdleState) {
                        if(APPLY_DELAY) {
                            delay += 1800;
                            if(FountainFamily.SHOW_UNLOCK_THANKS_MESSAGE) delay += 3700;
                    ...
                    //Wait a second to display the menu.
                    setTimeout(function() : void {

                        UNLOCK_SUCCESS_STATE = true;
                        UNLOCK_PROCESSING_STATE = false;
                        UNLOCK_ERROR_STATE = false;
                        UNLOCKED_TIME = __args[0] != null ? int(__args[0]) : 30; //30 seconds by default
                        UNLOCKED_REMAINING_TIME = __args[1] != null ? int(__args[1]) : 5; //30 seconds by default
                        FountainFamily.SHOW_UNLOCK_THANKS_MESSAGE = true;

                        countdownInterval = setInterval(function() : void {
                            dispatchEvents(new Event(EVENT_UPDATE_UNLOCK_COUNTDOWN));
                            UNLOCKED_TIME --;
                            if(UNLOCKED_TIME < 0) {
                                UNLOCKED_TIME = 0;
                                clearInterval(countdownInterval);
                                sendCommand(BACKEND_COMMAND_BEVERAGE_STOP);
                                sendCommand(BACKEND_COMMAND_UNLOCKED_SESSION_END);
                                dispatchEvents(new Event(EVENT_UNLOCKED_SESSION_FINISH));
                                BackendModel.UNLOCK_ERROR_STATE = false;
                                BackendModel.UNLOCK_PROCESSING_STATE = false;
                                BackendModel.UNLOCK_SUCCESS_STATE = false;
                            }
                        }, 1000);

                        dispatchEvents(new Event(EVENT_UNLOCKED_SUCCESS));
                        dispatchEvents(new Event(EVENT_UPDATE_UNLOCK_COUNTDOWN));
                    }, delay);
        ...
        private static function onUnlockedError(...__args) : void {
            if(UNLOCK_PROCESSING_STATE && !UNLOCK_ERROR_STATE) {

                setTimeout(function() : void {
                    UNLOCK_ERROR_STATE = true;
                    UNLOCK_PROCESSING_STATE = false;
                    UNLOCK_SUCCESS_STATE = false;
                    UNLOCKED_ERROR = __args[0] != null ? uint(__args[0]) : -1;
                    dispatchEvents(new Event(EVENT_UNLOCKED_ERROR));
		...
		private static function onInventoryChanged(...__args):void {
			...
			if (mustRefreshRecipes) {
				refreshRecipeAvailabilityInternal(true);
				if (hasRecipeAvailabilityChanged || DEBUG_RANDOM_INVENTORY_AVAILABILITY || true) {
        ...
        //Function used for the Unattended project.
        public function lockedPourCompleted() : void {
            UNLOCKED_TIME = 0;
            clearInterval(countdownInterval);
            stopPour();
            sendCommand(BACKEND_COMMAND_UNLOCKED_SESSION_END);
