package com.firstborn.pepsi.display.gpu.common {
	...
	public class TextureLibrary extends EventDispatcher {
		...
		private static const LOGO_DIMENSIONS_BIG:int = 512;						// Dimensions used by the ORIGINAL logo images (big version)
		...
		private function createTextureMessages():void {
			...
			for(var i : uint = 0; i < FountainFamily.LOCALE_ISO.length; i++) {
				...
				_textureBlobMessageUnavailableTitle.push(TextBitmap.createTextures(
						new <TextBitmap>[
							new TextBitmap(StringList.getList(FountainFamily.LOCALE_ISO[i]).getString("home/brand-unavailable-title-1"), FountainFamily.LANGUAGES_FONTS[i] ? FontLibrary.EXTERNAL_REGULAR : FontLibrary.BOOSTER_FY_REGULAR, null, 36.87, NaN, 0xffffff, -1, 1, 1, -20, -20, TextSpriteAlign.CENTER, LOGO_DIMENSIONS),
							new TextBitmap(StringList.getList(FountainFamily.LOCALE_ISO[i]).getString("home/brand-unavailable-title-2"), FountainFamily.LANGUAGES_FONTS[i] ? FontLibrary.EXTERNAL_REGULAR : FontLibrary.BOOSTER_FY_REGULAR, null, 59.91, NaN, 0xffffff, -1, 1, 1, -40, -40, TextSpriteAlign.CENTER, LOGO_DIMENSIONS),
						], -4, true, true
				));
				...
				_textureBlobMessageUnavailableSubtitle.push(TextBitmap.createTextures(
						new <TextBitmap>[
							new TextBitmap(StringList.getList(FountainFamily.LOCALE_ISO[i]).getString("home/brand-unavailable-subtitle"), FountainFamily.LANGUAGES_FONTS[i] ? FontLibrary.EXTERNAL_REGULAR : FontLibrary.BOOSTER_FY_REGULAR, null, 17.28, NaN, 0xffffff, -1, 1, 1, 60, 60, TextSpriteAlign.CENTER, LOGO_DIMENSIONS),
						], 0, true, true
				));
