		...
		public function BrandView(__options:BrandViewOptions, __beverageId:String, __language: uint = 0) {
			...
			if (!beverage.isMix) {
				...
				if (options.titleInTwoLines) {
					...
					for(i = 0; i < FountainFamily.LOCALE_ISO.length; i ++) {
						...
						if (!FountainFamily.objectRecycler.has(logoImageTextTitle1)) FountainFamily.objectRecycler.putNew(logoImageTextTitle1, new Image(TextBitmap.createTexture(titleText1.split("[[flavors]]").join(numMaxFlavors), FountainFamily.LANGUAGES_FONTS[i] ? FontLibrary.EXTERNAL_REGULAR : FontLibrary.BOOSTER_FY_REGULAR, null, options.fontSizeTitle, NaN, colorTextNormal & 0xffffff, -1, 1, 1, -25, -25, TextSpriteAlign.CENTER, false)));
						...
						if (!FountainFamily.objectRecycler.has(logoImageTextTitle2)) FountainFamily.objectRecycler.putNew(logoImageTextTitle2, new Image(TextBitmap.createTexture(titleText2.split("[[flavors]]").join(numMaxFlavors), FountainFamily.LANGUAGES_FONTS[i] ? FontLibrary.EXTERNAL_REGULAR : FontLibrary.BOOSTER_FY_REGULAR, null, options.fontSizeTitleEmphasis, NaN, colorTextStrong & 0xffffff, -1, 1, 1, -25, -25, TextSpriteAlign.CENTER, false)));
            ...
            if(Calories.CALORIES_ACTIVE) {
                ...
                for(i = 0; i < Calories.CUPS.length; i ++) {
                    ...
                    if (!FountainFamily.objectRecycler.has(caloriesAsset)) FountainFamily.objectRecycler.putNew(caloriesAsset, new Image(TextBitmap.createTexture(caloriesText, FountainFamily.LANGUAGES_FONTS[i] ? FontLibrary.EXTERNAL_BOLD : FontLibrary.BOOSTER_NEXT_FY_BOLD, null, caloriesFontSize, NaN, caloriesColor, caloriesColor, 1, 1, -25, -25, TextSpriteAlign.RIGHT)));
                ...
                if (!FountainFamily.objectRecycler.has(logoImageTextTitle)) FountainFamily.objectRecycler.putNew(logoImageTextTitle, new Image(TextBitmap.createTexture(titleText, FountainFamily.LANGUAGES_FONTS[i] ? FontLibrary.EXTERNAL_REGULAR : FontLibrary.BOOSTER_FY_REGULAR, null, options.fontSizeFlavorCalories, NaN, colorTextNormal & 0xffffff, colorTextStrong & 0xffffff, 1, 1, -25, -25, TextSpriteAlign.CENTER, false, NaN, 0, null, 5)));
			...
			if (beverageDesign.imageLogoFlavorSponsor.length > 0) {
				...
				if (beverage.isMix) {
					...
					if (!FountainFamily.objectRecycler.has(sponsorTextImageId)) FountainFamily.objectRecycler.putNew(sponsorTextImageId, new Image(TextBitmap.createTexture(sponsorText, FountainFamily.LANGUAGES_FONTS[i] ? FontLibrary.EXTERNAL_BOLD : FontLibrary.BOOSTER_NEXT_FY_BOLD, null, sponsorFontSizeMix, NaN, sponsorTextColor, -1, sponsorTextAlpha, 1, -25, -25, TextSpriteAlign.CENTER, false)));
