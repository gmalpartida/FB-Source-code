...
/**
	...
	public class FountainFamily extends SimpleApplication {
		...
		public static const DEBUG_MOUSE_VISIBLE									:Boolean = AppUtils.isDebugSWF() && true;
		...
		public static const NAME_ATTRIB_LABEL : String = "toggle-label";										//Attrib name in the localization.xml to define the label for each language
		public static const NAME_EXTERNAL_FONNT : String = "externalFont";										//Boolean value used to check if the font is external or the embedded
        ...
        public static var LANGUAGES_FONTS : Vector.<Boolean> = new Vector.<Boolean>();                            //Defines if the font for the corresponding language is the one embedded or an external one.
		...
		override protected function createVisualAssets():void {
            ...
            var requireExternalFont = false;
			for(i = 0; i < LOCALE_ISO.length; i ++) {
                LANGUAGES_LABELS.push(localeData.locale.(@id==LOCALE_ISO[i])[NAME_ATTRIB_LABEL]);
                LANGUAGES_FONTS.push(localeData.locale.(@id==LOCALE_ISO[i])[NAME_EXTERNAL_FONNT].toLowerCase() == "true");
                if(!requireExternalFont) requireExternalFont = localeData.locale.(@id==LOCALE_ISO[i])[NAME_EXTERNAL_FONNT].toLowerCase() == "true";
            }
			...
			if (FountainFamily.DEBUG_FULLSCREEN_WHEN_CLICKING_STAGE) {
				...
				stage.addEventListener(MouseEvent.CLICK, function(__e:Event):void {
					if (stage.displayState != StageDisplayState.FULL_SCREEN_INTERACTIVE) {
                        ...
                        if(requireExternalFont) loadExternalFont();
                        else main.init();
        ...
        private function loadExternalFont() {
            var $loader:Loader = new Loader();
            $loader.contentLoaderInfo.addEventListener(Event.COMPLETE, externalFontLoaded, false, 0, true);
            $loader.load(new URLRequest("./assets/fonts/externalFonts.swf"));
        }

        private function externalFontLoaded(e:Event):void {
            var FontLibrary:Class = e.target.applicationDomain.getDefinition(getQualifiedClassName(e.target.content)) as Class;
            Font.registerFont(FontLibrary.fontEL); //external light
            Font.registerFont(FontLibrary.fontEM); //external medium
            Font.registerFont(FontLibrary.fontER); //external regular
            Font.registerFont(FontLibrary.fontEB); //external bold
            main.init();
        }
