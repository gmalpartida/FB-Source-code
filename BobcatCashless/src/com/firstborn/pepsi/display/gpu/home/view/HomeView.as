...
import starling.display.Image;
	...
	public class HomeView extends Sprite {
        ...
        //Vector of images that contains the different required copys for calories in the Bobcat version
        private var bobcatCaloriesCopy : Vector.<Image> = new Vector.<Image>(FountainFamily.MAX_LANGUAGES);

        //Vector of images that contain the different required copy for the payment in the bobcat cashless POC
        private var paymentCopy : Vector.<Image> = new Vector.<Image>(FountainFamily.MAX_LANGUAGES);
        ...
        private const BOBCAT_ENABLED : Boolean = FountainFamily.platform.id == "bobcat";
		...
		public function HomeView(__options:HomeViewOptions, __forceDisallowKeyboardFocus:Boolean = false, __currentLanguage : uint = 1) {
			...
			if (options.hasTitle) {
                ...
                var bobcatCopyId : String = "";

				//Create the labels for all the languages
				for(i = 0; i < FountainFamily.LOCALE_ISO.length; i ++) {

                    var fontTypeTopTitle : String = FontLibrary.BOOSTER_FY_REGULAR;
                    if(options.fontTypeTopTitle == "bold") fontTypeTopTitle = FontLibrary.BOOSTER_NEXT_FY_BOLD;

                    var fontTypeBottomTitle : String = FontLibrary.BOOSTER_FY_REGULAR;
                    if(options.fontTypeBottomTitle == "bold") fontTypeBottomTitle = FontLibrary.BOOSTER_NEXT_FY_BOLD;

					imageTitleTopId = "HomeView_imageTitleTop_" + options.id + FountainFamily.LOCALE_ISO[i];
                    var tracking : Number = FountainFamily.platform.id == "bobcat" ? 80 : 160;
					if (!FountainFamily.objectRecycler.has(imageTitleTopId)) FountainFamily.objectRecycler.putNew(imageTitleTopId, new Image(TextBitmap.createTexture(StringList.getList(FountainFamily.LOCALE_ISO[i]).getString("home/title-top"), fontTypeTopTitle, null, options.fontSizeTopTitle, NaN, options.colorTopTitle, -1, 1, 1, tracking, tracking, TextSpriteAlign.CENTER, false)));
                    ...
                    if(BOBCAT_ENABLED) {
                        ...
                        if(FountainFamily.PAYMENT_ENABLED) {
                            bobcatCopyId = "HomeView_paymentCopy_" + options.id + FountainFamily.LOCALE_ISO[i];
                            if (!FountainFamily.objectRecycler.has(bobcatCopyId)) FountainFamily.objectRecycler.putNew(bobcatCopyId, new Image(TextBitmap.createTexture(FountainFamily.PAYMENT_AMOUNT.concat(" " + StringList.getList(FountainFamily.LOCALE_ISO[i]).getString("home/payment-copy")), fontTypeBottomTitle, null, 60, NaN, 0Xff4539, -1, 1, 1, 40, 40, TextSpriteAlign.CENTER, false, NaN, 0, null, 10)));
                            paymentCopy[i] = FountainFamily.objectRecycler.get(bobcatCopyId);
                            paymentCopy[i].touchable = false;
                            paymentCopy[i].smoothing = FountainFamily.platform.getTextureProfile(TextureLibrary.TEXTURE_ID_GENERIC_TEXT).smoothing;
                            paymentCopy[i].pivotX = paymentCopy[i].width * 0.5;
                            paymentCopy[i].x = options.width * 0.5;
                            addChild(paymentCopy[i]);
                            paymentCopy[i].visible = false;
                        }


                        bobcatCopyId = "HomeView_bobcatCopy_" + options.id + FountainFamily.LOCALE_ISO[i];
                        if (!FountainFamily.objectRecycler.has(bobcatCopyId)) FountainFamily.objectRecycler.putNew(bobcatCopyId, new Image(TextBitmap.createTexture(StringList.getList(FountainFamily.LOCALE_ISO[i]).getString("home/calories-copy"), fontTypeBottomTitle, null, 40, NaN, options.colorBottomTitle, -1, 1, 1, 40, 40, TextSpriteAlign.CENTER, false, NaN, 0, null, 10)));
                        bobcatCaloriesCopy[i] = FountainFamily.objectRecycler.get(bobcatCopyId);
                        bobcatCaloriesCopy[i].touchable = false;
                        bobcatCaloriesCopy[i].smoothing = FountainFamily.platform.getTextureProfile(TextureLibrary.TEXTURE_ID_GENERIC_TEXT).smoothing;
                        bobcatCaloriesCopy[i].pivotX = bobcatCaloriesCopy[i].width * 0.5;
                        bobcatCaloriesCopy[i].x = options.width * 0.5;
                        addChild(bobcatCaloriesCopy[i]);
                        bobcatCaloriesCopy[i].visible = false;
                    }
				}

				imageTitleTop[FountainFamily.DEFAULT_LANGUAGE].visible = true;
				imageTitleBottom[FountainFamily.DEFAULT_LANGUAGE].visible = true;
                if(BOBCAT_ENABLED) bobcatCaloriesCopy[FountainFamily.DEFAULT_LANGUAGE].visible = true;
                if(FountainFamily.PAYMENT_ENABLED) paymentCopy[FountainFamily.DEFAULT_LANGUAGE].visible = true;
			...
			if (options.buttonLayout != HomeViewOptions.BUTTON_LAYOUT_NONE) {
                ...
                if(options.waterButtonsEnabled) {
                    ...
                    if (!FountainFamily.objectRecycler.has(buttonPourTapWaterId)) {
                        blobButton = new BlobButton(
                                ...
                                [
                                    ...
                                    BlobButtonLayer.getSolidStrokeBlob(COLOR_BUTTON_FILL_COLOR, 1, FountainFamily.platform.mainColorInterface, 1, buttonStyle.strokeWidths[0])
                                ],
		...
		private function redrawVisibility():void {
			...
			for(var i : uint = 0; i < FountainFamily.LOCALE_ISO.length; i ++) {
                ...
                if(BOBCAT_ENABLED && bobcatCaloriesCopy[i] != null) {
                    bobcatCaloriesCopy[i].alpha = _uiVisibility;

                    if(FountainFamily.PAYMENT_ENABLED) {
                        paymentCopy[i].y = MathUtils.map(Equations.expoOut(_uiVisibility), 0, 1, - bobcatCaloriesCopy[i].height, options.marginTitleTop + imageTitleTop[i].height + options.marginTitleBottom + 95);
                        bobcatCaloriesCopy[i].y = MathUtils.map(Equations.expoOut(_uiVisibility), 0, 1, - bobcatCaloriesCopy[i].height, options.marginTitleTop + imageTitleTop[i].height + paymentCopy[i].height + bobcatCaloriesCopy[i].height + options.marginTitleBottom + 30);

                        if(FountainFamily.PAYMENT_ENABLED) paymentCopy[i].alpha = _uiVisibility;

                    } else {
                        bobcatCaloriesCopy[i].y = MathUtils.map(Equations.expoOut(_uiVisibility), 0, 1, - bobcatCaloriesCopy[i].height, options.marginTitleTop + imageTitleTop[i].height + bobcatCaloriesCopy[i].height + options.marginTitleBottom );
                    }
...
//		}
		...
		public function set language(value : uint) : void {
			for(var i: uint = 0; i < FountainFamily.LOCALE_ISO.length; i ++) {
                ...
                if(BOBCAT_ENABLED && bobcatCaloriesCopy[i] != null) bobcatCaloriesCopy[i].visible = false;
                if(FountainFamily.PAYMENT_ENABLED && paymentCopy[i] != null) paymentCopy[i].visible = false;
			}
            ...
            if(BOBCAT_ENABLED && bobcatCaloriesCopy[value] != null) bobcatCaloriesCopy[value].visible = true;
            if(FountainFamily.PAYMENT_ENABLED && paymentCopy[value] != null) paymentCopy[value].visible = true;
        ...
        public function get waterButtonHeight() : Number {
            return buttonPourSparklingWater != null ? buttonPourSparklingWater.y : 0;
        }
