...
import starling.display.BlendMode;
	...
	public class BrandView extends Sprite {
        ...
        //Text for the payment method intro
        private var paymentSwipeCopy : Vector.<Image> = new Vector.<Image>(10);
        private var paymentAuthorizeCopy : Vector.<Image> = new Vector.<Image>(10);
        private var paymentErrorCopy : Vector.<Image> = new Vector.<Image>(10); //This would have all the possible errors in all the possible languages
        private var paymentAuthorized : Boolean = false;
        private var paymentError : Boolean = false;
        private var paymentStarted : Boolean = false;
        private var buttonDone:BlobButton;

        private var paymentTimeLeftCopy : Vector.<Image> = new Vector.<Image>(10);
        private var timeLeftCopyTimer : uint;
        private var authorizeCircle : Image;
        private var authorizeInterval : uint;
		...
		public function BrandView(__options:BrandViewOptions, __beverageId:String, __language: uint = 0) {
			...
			// Pour button
            var requiresPayment : Boolean = FountainFamily.PAYMENT_ENABLED && _beverage.requiresPayment;
			...
			var buttonPourId:String = "BrandViewTower_buttonPour_" + beverageDesign.colorPourFill + "_" + beverageDesign.colorPourStroke + String(requiresPayment);
            ...
            //For the Bobcat Cashless project
            if(requiresPayment) {

                //Different copy For the Bobcat Cashless project
                var paymentStartId:String = "";
                var paymentStartCopy : String = "";

                for(i = 0; i < FountainFamily.LOCALE_ISO.length; i ++) {
                    paymentStartId = "BrandViewPaymentStart"  + "_" + options.fontSizeTitle + FountainFamily.LOCALE_ISO[i];
                    paymentStartCopy = StringList.getList(FountainFamily.LOCALE_ISO[i]).getString("brand/payment-init");
                    if (!FountainFamily.objectRecycler.has(paymentStartId)) FountainFamily.objectRecycler.putNew(paymentStartId, new Image(TextBitmap.createTexture(paymentStartCopy, FontLibrary.BOOSTER_NEXT_FY_BOLD, null, 40, NaN, FountainFamily.platform.mainColorInterface & 0xffffff, -1, 1, 1, 0, 0, TextSpriteAlign.CENTER, false)));
                    paymentSwipeCopy[i] = FountainFamily.objectRecycler.get(paymentStartId);
                    addChild(paymentSwipeCopy[i]);
                    paymentSwipeCopy[i].visible = false;
                    paymentSwipeCopy[i].x = 0.5 * (width - paymentSwipeCopy[i].width);
                    paymentSwipeCopy[i].y = options.height - pourButtonStyle.margin - buttonPour.radius;


                }
                paymentSwipeCopy[currentLanguage].visible = true;

                var paymentAuthorizeId:String = "";
                var paymentAuthorizeText : String = "";

                for(i = 0; i < FountainFamily.LOCALE_ISO.length; i ++) {
                    paymentAuthorizeId = "BrandViewPaymentAuthorize"  + "_" + options.fontSizeTitle + FountainFamily.LOCALE_ISO[i];
                    paymentAuthorizeText = StringList.getList(FountainFamily.LOCALE_ISO[i]).getString("brand/payment-authorize");
                    if (!FountainFamily.objectRecycler.has(paymentStartId)) FountainFamily.objectRecycler.putNew(paymentAuthorizeId, new Image(TextBitmap.createTexture(paymentAuthorizeText, FontLibrary.BOOSTER_NEXT_FY_BOLD, null, 30, NaN, FountainFamily.platform.mainColorInterface & 0xffffff, -1, 1, 1, 0, 0, TextSpriteAlign.CENTER, false)));
                    paymentAuthorizeCopy[i] = FountainFamily.objectRecycler.get(paymentAuthorizeId);
                    addChild(paymentAuthorizeCopy[i]);
                    paymentAuthorizeCopy[i].visible = false;
                    paymentAuthorizeCopy[i].x = 0.5 * (width - paymentAuthorizeCopy[i].width);
                    paymentAuthorizeCopy[i].y = options.height - pourButtonStyle.margin - buttonPour.radius - (options.isADA ? 40 : 0);;
                }

                var paymentErrorId:String = "";
                var paymentErrorText : String = "";

                for(i = 0; i < FountainFamily.LOCALE_ISO.length; i ++) {
                    paymentErrorId = "BrandViewPaymentError"  + "_" + options.fontSizeTitle + FountainFamily.LOCALE_ISO[i];
                    paymentErrorText = StringList.getList(FountainFamily.LOCALE_ISO[i]).getString("brand/payment-error");
                    if (!FountainFamily.objectRecycler.has(paymentStartId)) FountainFamily.objectRecycler.putNew(paymentErrorId, new Image(TextBitmap.createTexture(paymentErrorText, FontLibrary.BOOSTER_NEXT_FY_BOLD, null, 40, NaN, FountainFamily.platform.mainColorInterface & 0xffffff, -1, 1, 1, 0, 0, TextSpriteAlign.CENTER, false)));
                    paymentErrorCopy[i] = FountainFamily.objectRecycler.get(paymentErrorId);
                    addChild(paymentErrorCopy[i]);
                    paymentErrorCopy[i].visible = false;
                    paymentErrorCopy[i].x = 0.5 * (width - paymentErrorCopy[i].width);
                    paymentErrorCopy[i].y = options.height - pourButtonStyle.margin - buttonPour.radius;
                }

                // Done button
                languagesStrings.length = 0;
                for(j = 0; j < FountainFamily.LOCALE_ISO.length; j ++) languagesStrings.push(StringList.getList(FountainFamily.LOCALE_ISO[j]).getString("brand/payment-done"));
                var buttonDoneId:String = "BrandViewTower_buttonDone_" + (options.buttonBackColorsNeutral ? "_neutral" : beverageDesign.colorButtonsFill + "_" + beverageDesign.colorButtonsStroke) + "_" + options.allowKeyboardFocus;
                if (!FountainFamily.objectRecycler.has(buttonDoneId)) {
                    if (options.buttonBackColorsNeutral) {
                        // Neutral colors (ADA)
                        blobButton = new BlobButton(
                                buttonStyle.radius,
                    ...
                    FountainFamily.objectRecycler.putNew(buttonDoneId, blobButton);
                    blobButton = null;
                }

                buttonDone = FountainFamily.objectRecycler.get(buttonDoneId);
                buttonDone.recycle();
                buttonDone.start();
                buttonDone.x = width - (buttonStyle.margin + buttonDone.radius);
                buttonDone.y = options.height - buttonStyle.margin - buttonDone.bottomHeight;
                buttonDone.onTapped.add(onButtonDoneTapped);
                buttonDone.imgCaption.pivotY = buttonDone.imgCaption.texture.height * 0.60; // Lee: moved text up
                buttonDone.imgIcon.pivotY = buttonDone.imgIcon.texture.height * 0.35;  // Lee: moved icon down due to adjusted radius
                addChild(buttonDone);
                buttonDone.visibility = 0;
                buttonDone.visible = false;

                if (options.allowKeyboardFocus) FountainFamily.focusController.addElement(buttonDone);

                buttonPour.visibility = 0;
                buttonPour.visible = false;
                buttonPour.timerAngle = 0;

                var logoThanksId:String = "BrandViewLogoThanks";
                if (!FountainFamily.objectRecycler.has(logoThanksId)) FountainFamily.objectRecycler.putNew(logoThanksId, new Image(FountainFamily.textureLibrary.getAquafinaLogoThanks()));

                var authorizeCircleId:String = "BrandViewAuthorizePending";
                if (!FountainFamily.objectRecycler.has(authorizeCircleId)) FountainFamily.objectRecycler.putNew(authorizeCircleId, new Image(FountainFamily.textureLibrary.getAuthorizationPending()));

                authorizeCircle = FountainFamily.objectRecycler.get(authorizeCircleId);
                authorizeCircle.scaleX = authorizeCircle.scaleY = 1;
                authorizeCircle.pivotX = authorizeCircle.width * 0.5;
                authorizeCircle.pivotY = authorizeCircle.height * 0.5;
                authorizeCircle.y = paymentAuthorizeCopy[currentLanguage].y + paymentAuthorizeCopy[currentLanguage].height * 0.5;
                authorizeCircle.x = paymentAuthorizeCopy[currentLanguage].x + paymentAuthorizeCopy[currentLanguage].width * 0.5;
                authorizeCircle.visible = false;
                addChild(authorizeCircle);

                var animationTime : Number = 0.4;

                FountainFamily.backendModel.addEventListener(BackendModel.EVENT_PAYMENT_AUTHORIZE, function() : void {

                    swapChildren(paymentAuthorizeCopy[currentLanguage], authorizeCircle);

                    if(!paymentStarted) {
                        paymentStarted = true;
                        paymentAuthorizeCopy[currentLanguage].alpha = 0;
                        paymentAuthorizeCopy[currentLanguage].visible = true;
                        authorizeCircle.visible = true;
                        authorizeCircle.alpha = 0;


                        ZTween.add(buttonBack, {visibility:0}, {time:animationTime});
                        ZTween.add(paymentSwipeCopy[currentLanguage], {alpha:0}, {time:animationTime});
                        ZTween.add(paymentAuthorizeCopy[currentLanguage], {alpha:1}, {time:animationTime, delay:animationTime, onUpdate: function() : void {
                            authorizeCircle.alpha = paymentAuthorizeCopy[currentLanguage].alpha;
                        }});
                    }

                    authorizeInterval = setInterval(function() : void {
                        authorizeCircle.rotation += 0.1;
                    }, 24);

                });

                FountainFamily.backendModel.addEventListener(BackendModel.EVENT_PAYMENT_SUCCESS, function() : void {

                    if(paymentStarted && !paymentError) {

                        clearInterval(authorizeInterval);

                        for(i = 0; i < FountainFamily.LOCALE_ISO.length; i ++) {
                            if(paymentTimeLeftCopy[0] != null) {
                                removeChild(paymentTimeLeftCopy[i]);
                                paymentTimeLeftCopy[i] = null;
                            }
                        }

                        var paymentTimeLeftId:String = "";
                        var paymentTimeLeftText : String = "";

                        for(i = 0; i < FountainFamily.LOCALE_ISO.length; i ++) {
                            paymentTimeLeftId = "BrandViewPaymentTimeLeft"  + "_" + options.fontSizeTitle + FountainFamily.LOCALE_ISO[i] + String(BackendModel.POURING_REMAINING_TIME_SECONDS);
                            paymentTimeLeftText = String(BackendModel.POURING_REMAINING_TIME_SECONDS).concat(" " + StringList.getList(FountainFamily.LOCALE_ISO[i]).getString("brand/payment-timeLeft"));
                            if (!FountainFamily.objectRecycler.has(paymentStartId)) FountainFamily.objectRecycler.putNew(paymentTimeLeftId, new Image(TextBitmap.createTexture(paymentTimeLeftText, FontLibrary.BOOSTER_NEXT_FY_BOLD, null, 40, NaN, FountainFamily.platform.mainColorInterface & 0xffffff, -1, 1, 1, 0, 0, TextSpriteAlign.CENTER, false)));
                            paymentTimeLeftCopy[i] = FountainFamily.objectRecycler.get(paymentTimeLeftId);
                            addChild(paymentTimeLeftCopy[i]);
                            paymentTimeLeftCopy[i].visible = false;
                            paymentTimeLeftCopy[i].x = 0.5 * (width - paymentTimeLeftCopy[i].width);
                            paymentTimeLeftCopy[i].y = options.height - pourButtonStyle.margin - 2.5 * buttonPour.radius;
                        }

                        buttonDone.visible = true;
                        buttonPour.visible = true;

                        ZTween.add(paymentAuthorizeCopy[currentLanguage], {alpha:0}, {time: animationTime, onUpdate: function() : void {
                            authorizeCircle.alpha = paymentAuthorizeCopy[currentLanguage].alpha;
                        }});

                        ZTween.add(buttonPour, {visibility:1}, {time: animationTime, delay: animationTime});
                        ZTween.add(buttonDone, {visibility:1}, {time: animationTime, delay: animationTime});

                        //Animation for the pour button (timer for pouring
                        ZTween.remove(buttonPour, "timerAngle");
                        ZTween.add(buttonPour, {timerAngle: 360}, {time: BackendModel.POURING_TIME_SECONDS, delay: (animationTime * 2), transition:Equations.none, onComplete:onButtonDoneTapped, onCompleteParams:[null],
                        onStart: function() : void {
                            timeLeftCopyTimer = setTimeout(function() : void {
                                paymentTimeLeftCopy[currentLanguage].visible = true;
                                paymentTimeLeftCopy[currentLanguage].alpha = 1;
                                ZTween.add(paymentTimeLeftCopy[currentLanguage], {alpha:0}, {time: 1.3});
                            }, (BackendModel.POURING_TIME_SECONDS - BackendModel.POURING_REMAINING_TIME_SECONDS) * 1000);
                        }});
                ...
                FountainFamily.backendModel.addEventListener(BackendModel.EVENT_PAYMENT_ERROR, function() : void {

                    if(paymentStarted && !paymentAuthorized) {
                        clearInterval(authorizeInterval);
                        paymentErrorCopy[currentLanguage].alpha = 0;
                        paymentErrorCopy[currentLanguage].visible = true;

                        ZTween.add(buttonBack, {visibility:1}, {time:animationTime});
                        ZTween.add(paymentAuthorizeCopy[currentLanguage], {alpha:0}, {time: animationTime, onUpdate: function() : void {
                            authorizeCircle.alpha = paymentAuthorizeCopy[currentLanguage].alpha;
                        }});
                        ZTween.add(paymentErrorCopy[currentLanguage], {alpha:1}, {time:animationTime, delay:animationTime});
            ...
            //To make it appear on top of all the copy
            addChild(buttonPour);
		...
		private function redrawVisibility():void {
            ...
            if(FountainFamily.PAYMENT_ENABLED && _beverage.requiresPayment) {
                paymentSwipeCopy[currentLanguage].alpha = uiPhase;
                paymentErrorCopy[currentLanguage].alpha = uiPhase;
                if(paymentAuthorized ) {
                    paymentSwipeCopy[currentLanguage].alpha = 0;
                    buttonPour.visibility = uiPhase;
                    buttonDone.visibility = uiPhase;
                    buttonBack.visibility = 0;
                }
                if(paymentError) {
                    paymentAuthorizeCopy[currentLanguage].alpha = 0;
                    paymentSwipeCopy[currentLanguage].alpha = 0;
        ...
        private function onButtonDoneTapped(__button:BlobButton) : void {
            ZTween.remove(buttonPour, "timerAngle");
            clearTimeout(timeLeftCopyTimer);
            FountainFamily.attractorController.goToIdleState();
            FountainFamily.backendModel.paymentPourCompleted();
        }
		...
		override public function dispose():void {
            ...
            //For the Cashless
            if(_beverage.requiresPayment && FountainFamily.PAYMENT_ENABLED) {

                FountainFamily.focusController.removeElement(buttonDone);
                removeChild(buttonDone);
                buttonDone.onTapped.remove(onButtonDoneTapped);
                buttonDone.stop();
                FountainFamily.objectRecycler.putBack(buttonDone);
                buttonDone = null;

                for(j = 0; j < FountainFamily.LOCALE_ISO.length; j ++) {

                    if (paymentSwipeCopy[j] != null) {
                        removeChild(paymentSwipeCopy[j]);
                        FountainFamily.objectRecycler.putBack(paymentSwipeCopy[j]);
                        paymentSwipeCopy[j] = null;
                    }

                    if (paymentAuthorizeCopy[j] != null) {
                        removeChild(paymentAuthorizeCopy[j]);
                        FountainFamily.objectRecycler.putBack(paymentAuthorizeCopy[j]);
                        paymentAuthorizeCopy[j] = null;
                    }

                    if (paymentErrorCopy[j] != null) {
                        removeChild(paymentErrorCopy[j]);
                        FountainFamily.objectRecycler.putBack(paymentErrorCopy[j]);
                        paymentErrorCopy[j] = null;
                    }

                    if (paymentTimeLeftCopy[j] != null) {
                        removeChild(paymentTimeLeftCopy[j]);
                        FountainFamily.objectRecycler.putBack(paymentTimeLeftCopy[j]);
                        paymentTimeLeftCopy[j] = null;
                    }
                }
            }
