...
import starling.display.Quad;
	...
	public class GPURoot extends Sprite {
		...
		private static const TIME_BRAND_SHOW:Number = 1.3 * (FountainFamily.DEBUG_MAKE_BRAND_VIEW_TRANSITION_SLOW ? 6 : 1);
		private static const TIME_BRAND_HIDE:Number = 1.1 * (FountainFamily.DEBUG_MAKE_BRAND_VIEW_TRANSITION_SLOW ? 6 : 1);
		...
		public function GPURoot() {
            ...
            FountainFamily.backendModel.addEventListener(BackendModel.EVENT_PAYMENT_AUTHORIZE, function() : void {
                if(buttonSoftwareADA != null && softwareADAVisibility > 0) ZTween.add(buttonSoftwareADA, {visibility:0}, {time:TIME_SOFTWARE_ADA_SHOW});
            });

            FountainFamily.backendModel.addEventListener(BackendModel.EVENT_PAYMENT_POUR_COMPLETE, function() : void {
                if(buttonSoftwareADA != null && softwareADAVisibility > 0) ZTween.add(buttonSoftwareADA, {visibility:1}, {time:TIME_SOFTWARE_ADA_SHOW});
            });
...
//            }
		...
		public function init(__testingOverlay:TestingOverlay):void {
            ...
            //Insert the GPU background image if required
            if(FountainFamily.platform.backgroundImage != "") {
                var loader:Loader = new Loader();
                var backgroundContainer : Sprite = new Sprite();
                addChild(backgroundContainer);

                loader.contentLoaderInfo.addEventListener(Event.COMPLETE, function(e : Event) : void {
                    var background : Image = new Image(Texture.fromBitmap(Bitmap(LoaderInfo(e.target).content)));
                    backgroundContainer.addChild(background);
                });

                loader.contentLoaderInfo.addEventListener(IOErrorEvent.IO_ERROR, function(e: Event) : void {
                    trace("Background image URL not found");
                });


                loader.load(new URLRequest(FountainFamily.platform.backgroundImage));
            }
			...
			if (canActivateADA) {
				...
				buttonSoftwareADA = new BlobButton(
						...
						false, null, new ImageLibrary.ICON_ADA(), FountainFamily.platform.mainColorInterface, buttonStyle.iconScale);
				buttonSoftwareADA.x = w - buttonStyle.margin - buttonSoftwareADA.radius;

                rightAdaPosition = buttonSoftwareADA.x;
                leftAdaPosition = buttonSoftwareADA.x; //options.marginADAButtonLeft;
