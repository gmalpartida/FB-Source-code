package com.firstborn.pepsi.display.flash {
	...
	public class RichMessageOverlay extends Sprite {
		...
		public function RichMessageOverlay() {
			...
			textTitle = TextBitmap.createSprite(StringList.getList(FountainFamily.current_language).getString("generic/out-of-service/title"), FontLibrary.BOOSTER_FY_REGULAR, null, 76, NaN, FountainFamily.platform.mainColorInterface, -1, 1, 1, 60, 60, TextSpriteAlign.CENTER, w * 0.8);
			textTitle.x = w * 0.5 - textTitle.width * 0.5;
			textTitle.y = bitmapIcon.y - 35 - textTitle.height;
			addChild(textTitle);

			textBody = TextBitmap.createSprite(StringList.getList(FountainFamily.current_language).getString("generic/out-of-service/body"), FontLibrary.BOOSTER_FY_REGULAR, null, 30, NaN, FountainFamily.platform.mainColorInterface, FountainFamily.platform.mainColorInterface, 1, 1, 55, 55, TextSpriteAlign.CENTER, w * 0.8, 8);
			textBody.x = w * 0.5 - textBody.width * 0.5;
			textBody.y = bitmapIcon.y + bitmapIcon.height + 35;
			addChild(textBody);

			textMessage = TextBitmap.createSprite("", FontLibrary.BOOSTER_FY_REGULAR, null, 20, NaN, FountainFamily.platform.mainColorInterface, -1, 1, 1, 75, 75, TextSpriteAlign.RIGHT);
