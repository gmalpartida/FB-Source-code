package com.zehfernando.utils {
	...
	public class VectorUtils {
		...
		public static function stringToStringVector(__string:String, __separator:String, __trim:Boolean = false):Vector.<String> {
			var v:Vector.<String> = new Vector.<String>();
			if (Boolean(__string) && __string.length > 0) {
				var stringList:Array = __string.split(__separator);
				for (var i:int = 0; i < stringList.length; i++) v.push(__trim ? StringUtils.trim(stringList[i]) : stringList[i]);
			}
			return v;
		}
