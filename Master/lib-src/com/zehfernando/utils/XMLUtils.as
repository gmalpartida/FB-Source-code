package com.zehfernando.utils {
	...
	public class XMLUtils {
		...
		public static function getNodeAsString(__xml:XML, __nodeName:String, __default:String = ""):String {
			// Returns the value of a node. If it's simple (<name>value</name>), return the internal value ("value"); if it has subtags (<name><first>value</first></name>), returns a full XML with the parent name too ("<name><first>value</first></name>")
			if (!Boolean(__xml)) return __default;
		...
		public static function getNodeAsStringInternal(__xml:XML, __nodeName:String, __default:String = ""):String {
			// Returns the value of a node. If it's simple (<name>value</name>), return the internal value ("value"); if it has subtags (<name><first>value</first></name>), returns the internal value ("<first>value</first>")
			if (!Boolean(__xml)) return __default;
			var __firstChild:XML = getFirstNode(__xml.child(__nodeName));
			var __nodeValue:String;
			if (__firstChild.hasComplexContent()) {
				// Complex, strips out start/end tag
				__nodeValue = StringUtils.getCleanString(__firstChild.children().toXMLString());
			} else {
				// Simple, just return it
				__nodeValue = StringUtils.getCleanString(__firstChild);
			}
			if (Boolean(__nodeValue)) return __nodeValue;
			return __default;
		}
		...
		protected static function getNodeValue(__xmlList:XMLList):String {
			var str:String = "";
			if (__xmlList != null && __xmlList.length() > 0) str = StringUtils.getCleanString(__xmlList[0]);
			return str;
		}
