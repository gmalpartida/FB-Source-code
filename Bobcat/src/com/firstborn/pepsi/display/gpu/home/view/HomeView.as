		...
		private function redrawVisibility():void {
			...
			for(var i : uint = 0; i < FountainFamily.LOCALE_ISO.length; i ++) {
                ...
                if(BOBCAT_ENABLED && bobcatCaloriesCopy[i] != null) {
                    ...
                    bobcatCaloriesCopy[i].y = MathUtils.map(Equations.expoOut(_uiVisibility), 0, 1, - bobcatCaloriesCopy[i].height, options.marginTitleTop + imageTitleTop[i].height + bobcatCaloriesCopy[i].height + options.marginTitleBottom + 30);
                }
			}
