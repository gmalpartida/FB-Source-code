...
/**
	...
	public class Main extends flash.display.Sprite {
		...
		//Variables for the video object used in the stream
		private var camViewer : Video;
		private var videoEnabled : Boolean = true;

		//Connection for the remote video (Comedian Stream to show)
		private var _videoInConnection : NetConnection= new NetConnection();
		private var _videoInStream : NetStream = null;

		//Connection for the local video to send (user stream)
		private var _videoOutConnection : NetConnection= new NetConnection();
		private var _videoOutStream : NetStream = null;

		//Camera and microphone to use for the video chat
		...
		private var liveStreamStatus : Boolean = false;
		...
		private function hideGPURoot():void {
			...
			if (FountainFamily.platform.supportsLightsAPI) {
				FountainFamily.backendModel.setLightColorARGB(FountainFamily.attractorInfo.getColorAt(0), TIME_FADE_TO_IDLE * 1000, FountainFamily.lightingInfo.brightnessScale);
			}
		...
		private function showAttractorVideo():void {
			...
			attractorVideo.setChatState(videoEnabled);
			attractorVideo.bufferTime = 0.1;
			attractorVideo.autoPlay = true;
			attractorVideo.load(FountainFamily.attractorInfo.file);

			if(_videoInStream != null && liveStreamStatus) {
				attractorVideo.setLiveStream(_videoInStream);
			}
		...
		private function onEnterFrameUpdateAttractorLightColor(__e:flash.events.Event):void {
			...
			if (attractorVideoCover.alpha > 0) {
				...
				FountainFamily.backendModel.setLightColorARGB(Color.interpolateAARRGGBB(FountainFamily.lightingInfo.colorStandby, attractorColor, attractorVideoCover.alpha), 0, FountainFamily.lightingInfo.brightnessScale);
			} else {
				// Attractor color
				FountainFamily.backendModel.setLightColorARGB(attractorColor, 0, FountainFamily.lightingInfo.brightnessScale);
			}
		}
		...
		private function initFlashLayer():void {
			...
			/*
			* These commands initiate the required variables for the Crashers video chat.
			*/

			if(FountainFamily.STREAMING_ENABLED) {

                try {
                    //Video element for the menu and brand views.
                    ...
                    //Grab the camera and microphone for the chat
                    camera = Camera.getCamera();
                    mic = Microphone.getMicrophone();

                    camera.setMode(600, 960, 15, true);
                    camera.setQuality(125000, 85);
                    camera.setKeyFrameInterval(15);

                    //Connection to the video from the remote client
                    _videoInConnection.addEventListener(NetStatusEvent.NET_STATUS, onNetVideoInStatus);
                    _videoInConnection.connect(FountainFamily.REMOTE_STREAM_URL);

                    //Connection with the machine and the server to send the local video.
                    _videoOutConnection.addEventListener(NetStatusEvent.NET_STATUS, onNetVideoOutStatus);
                    _videoOutConnection.connect(FountainFamily.MACHINE_STREAM_URL);
			...
			//=======================================================================================================
			// END OF INITIALIZATION CODE FOR THE CRASHERS APP.
    ...
    private function generateBubble(size : Number) : void {
        ...
        if(size > 0) {
            camViewer = new Video(size * 2, size * 2);
            camViewer.smoothing = true;
            camViewer.cacheAsBitmap = true;
            ...
            try {
                if(videoContainer.visible) camViewer.attachNetStream(_videoInStream);
...
//				_wasFocusControllerUsed = false;
		...
		private function onGPURootHidden():void {
			...
			if(camViewer != null) videoContainer.visible = false;
			showAttractorVideo();
		}

		private function onGPURootShown():void {
			if(camViewer != null) {
                videoContainer.visible = videoEnabled && liveStreamStatus;
                camViewer.attachNetStream(_videoInStream);
            }
		...
		// ================================================================================================================
		// CRASHERS CODE FOR VIDEO CONFERENCE -----------------------------------------------------------------------------

		//Event fired when the connection to the remote stream is done with the server
		private function onNetVideoInStatus(e : NetStatusEvent) : void {
			switch(e.info.code) {
				case "NetConnection.Connect.Success":
					trace("Connection made to the remote stream");
					_videoInStream = new NetStream(_videoInConnection);
					_videoInStream.addEventListener(NetStatusEvent.NET_STATUS, checkVideoInStatus);
					_videoInStream.checkPolicyFile = true;
					_videoInStream.play(FountainFamily.REMOTE_STREAM_NAME);
                    ...
                    client.videoPosition = function(data) : void {
						var info : Array = data.split(" ");
                        videoContainer.x = info[0];
                        videoContainer.y = info[1];
					}

                    client.bubbleSize = function(data) : void {
                        trace("modify bubble size: " + data);
                        generateBubble(Number(data));
                    }

					client.videoSize = function(data) : void {
						trace("receiving: " + String(data));
						...
						switch(data) {
							case "fullscreen":
								...
								break;
							case "smallSize":
								var e : MouseEvent = new MouseEvent(MouseEvent.MOUSE_UP, true, false, 0, 0, stage, false, false, false);
								dispatchEvent(e);
								break;
						}
		...
		//Event fired when the connection to the machine stream is done with the server
		private function onNetVideoOutStatus(e : NetStatusEvent) : void {
			switch(e.info.code) {
				case "NetConnection.Connect.Success":
					trace("Connection made to the machine stream");
					_videoOutStream = new NetStream(_videoOutConnection);
					_videoOutStream.addEventListener(NetStatusEvent.NET_STATUS, checkVideoOutStatus);
					_videoOutStream.checkPolicyFile = true;
					_videoOutStream.attachCamera(camera);
					_videoOutStream.attachAudio(mic);

					camera.setMode(360, 640, 12, true);
					camera.setQuality(0, 100);
					camera.setKeyFrameInterval(12);

					var h264Settings:H264VideoStreamSettings = new H264VideoStreamSettings();
					h264Settings.setProfileLevel( H264Profile.BASELINE, H264Level.LEVEL_3_1 )
					//_videoOutStream.videoStreamSettings = h264Settings;

					_metaData.codec = _videoOutStream.videoStreamSettings.codec;
...
//					_metaData.keyFrameInterval = camera.keyFrameInterval;
					...
					_videoOutStream.send( "@setDataFrame", "onMetaData", _metaData );
					_videoOutStream.publish(FountainFamily.MACHINE_STREAM_NAME);
                    ...
                    stage.addEventListener(MouseEvent.MOUSE_UP, function (e:MouseEvent):void {
                        ...
                        try {
                            if (e.target != centerer && e.target != videoContainer) _videoOutStream.send("upPosition", String(stage.mouseX).concat(" ").concat(stage.mouseY));
                        } catch (ev:Error) {
                    ...
                    stage.addEventListener(MouseEvent.MOUSE_DOWN, function (e:MouseEvent):void {
                        ...
                        try {
                            if (e.target != centerer && e.target != videoContainer) _videoOutStream.send("downPosition", String(stage.mouseX).concat(" ").concat(stage.mouseY));
                        } catch (ev:Error) {
		...
		//Status events for the remote stream (the one from the comedian)
		private function checkVideoInStatus(e : NetStatusEvent) : void {
			switch(e.info.code) {

				case "NetStream.Play.StreamNotFound":
					liveStreamStatus = false;
                    videoContainer.visible = false;
					if(attractorVideo != null) attractorVideo.setChatState(false);
					break;
				case "NetStream.Play.PublishNotify":
					if(gpuRoot.visibility == 1 && videoEnabled) FountainFamily.attractorController.goToIdleState();
					liveStreamStatus = true;
					if(attractorVideo != null && videoEnabled) attractorVideo.setLiveStream(_videoInStream);
				...
				case "NetStream.Play.UnpublishNotify":
					liveStreamStatus = false;
                    videoContainer.visible = false;
                    camViewer.attachNetStream(null);
                    if(attractorVideo != null) attractorVideo.setChatState(false);
                    FountainFamily.attractorInfo.delayBrand = 15;
                    FountainFamily.attractorInfo.delayBrandADA = 15;
                    FountainFamily.attractorInfo.delayHome = 10;
                    FountainFamily.attractorInfo.delayHomeADA = 10;
                    FountainFamily.attractorController.goToIdleState();
					break;
			}
		}

		//Status events for the machine stream
		private function checkVideoOutStatus(e : NetStatusEvent) : void {
			switch(e.info.code) {
				case "NetStream.Play.StreamNotFound":
					break;
				case "NetStream.Play.PublishNotify":
