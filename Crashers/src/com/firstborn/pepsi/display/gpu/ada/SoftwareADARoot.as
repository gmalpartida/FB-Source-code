...
//			addChild(mQuad);
			...
			for(j = 0; j < FountainFamily.LOCALE_ISO.length; j ++) {
				...
				if (!FountainFamily.objectRecycler.has(messageId)) FountainFamily.objectRecycler.putNew((messageId), new Image(TextBitmap.createTextures(new <TextBitmap>[new TextBitmap(StringList.getList(FountainFamily.LOCALE_ISO[j]).getString("ada/home/return-1"), FontLibrary.BOOSTER_FY_REGULAR, null, 21, NaN, 0x3da5e7, -1, 1, 1, 50, 50, TextSpriteAlign.CENTER, w * 0.7), new TextBitmap(StringList.getList(FountainFamily.LOCALE_ISO[j]).getString("ada/home/return-2"), FontLibrary.BOOSTER_FY_REGULAR, null, 26, NaN, 0x3da5e7, -1, 1, 1, 150, 150, TextSpriteAlign.CENTER, w * 0.7)], 4, false, true)));
		...
		private function redrawBrandViewVisibility():void {
			...
			if (_brandViewVisibility < 0.5 || brandView == null) {
				...
				clickableCover.height = homeView.y + homeView.getMenuRect().y;
