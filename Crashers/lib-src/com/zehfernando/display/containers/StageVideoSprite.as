package com.zehfernando.display.containers {
	...
	public class StageVideoSprite extends ResizableSprite {
		...
		//These variables are made for the real time chat
		public var liveStream : NetStream;
		private var chatState : Boolean = false;
		private var liveStreamActive : Boolean = false;
...
//			});
		...
		private function attachToStageVideo(__stageVideoIndex:int):void {
			...
			//Depending on the state of the chat one stream runs or the other.
			chatState ? _stageVideo.attachNetStream((liveStream != null && liveStreamActive)? liveStream : _netStream) : _stageVideo.attachNetStream(_netStream);
...
//			log("##### NET STREAM METADATA DATA : " + JSON.stringify(__newData));
		...
		public function setLiveStream(_stream : NetStream) : void {
			if(liveStream == null) {
				liveStream = _stream;
		...
		public function setChatState(state : Boolean) : void {

			//It the video from the attractor is being displayed.
			try {
				if(state) {
					_stageVideo.attachNetStream(null);
					_stageVideo.attachNetStream((liveStream != null) ? liveStream : _netStream);
				} else {
					_stageVideo.attachNetStream(null);
