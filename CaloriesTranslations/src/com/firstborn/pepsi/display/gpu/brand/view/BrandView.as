...
import starling.display.BlendMode;
	...
	public class BrandView extends Sprite {
		...
		private var imageTextTitleLine1: Vector.<Image> = new Vector.<Image>(50);
		private var imageTextTitleLine2: Vector.<Image> = new Vector.<Image>(50);

        //These vectors contains the calories
        private var imageTextCalories1 : Vector.<Image> = new Vector.<Image>(100);
        private var imageTextCalories2 : Vector.<Image> = new Vector.<Image>(100);
        private var totalCupsLocale : int = 0;

        //container for the calories copy
        private var caloriesSprite : Vector.<Sprite> = new Vector.<Sprite>(100);

        //Hack to place the text of the flavors.
        private var flavorsCaloriesCopy : Vector.<Image> = new Vector.<Image>(100);

        //beverage's name for the calories
        private var beverageName : Image;
		...
		public function BrandView(__options:BrandViewOptions, __beverageId:String, __language: uint = 0) {
            ...
            if(Calories.CALORIES_ACTIVE) {
                ...
                for(var u : uint = 0; u < FountainFamily.LOCALE_ISO.length; u++) {
                    caloriesSprite[u] = new Sprite();
                    addChild(caloriesSprite[u]);
                    caloriesSprite[u].visible = false;

                    for(i = 0; i < Calories.CUPS.length; i ++) {

                        caloriesText = "";
                        var cupName : String = String(Calories.CUPS[i].names[u]).toUpperCase();
                        if(cupName.length > 0) caloriesText += cupName + " ";

                        if(Calories.DISPLAY_ORIGINAL_MEASUREMENT ) {
                            caloriesText += Math.round(Calories.CUPS[i].size * Calories.MEASUREMENT_CONVERSION) + String(Calories.CUPS_MEASUREMENT).toUpperCase() + " (" + Math.round(Calories.CUPS[i].size) + "OZ)";
                        } else {
                            caloriesText += Math.round(Calories.CUPS[i].size * Calories.MEASUREMENT_CONVERSION) + String(Calories.CUPS_MEASUREMENT).toUpperCase();
                        }

                        caloriesAsset = "CaloriesImageTitle_#ffffff_" + String(Calories.CUPS[i].names[u]).toUpperCase() + "_" + Calories.CUPS[i].size + "_" + String(Calories.CUPS_MEASUREMENT).toUpperCase() + "_" + caloriesColor;
                        ...
                        imageTextCalories1[totalCupsLocale] = FountainFamily.objectRecycler.get(caloriesAsset);
                        caloriesSprite[u].addChild(imageTextCalories1[totalCupsLocale]);

                        //Execute the rounding rules for the calories
                        var caloriesValue : Number = _beverage.calories * Calories.CUPS[i].size;
                        for(var r : int = 0; r < Calories.ROUNDING_RULES.length; r ++) {
                            var partial : Number = Calories.ROUNDING_RULES[r].roundTo(caloriesValue);
                            if(caloriesValue != partial) {
                                caloriesValue = partial;
                                break;
                            }
                        }

                        caloriesText = String(caloriesValue) + " " + StringList.getList(FountainFamily.LOCALE_ISO[u]).getString("calories/calories_acronym");
                        caloriesAsset = "CaloriesImageTitle_" + String(caloriesValue) + "_CAL" + "_" + caloriesColor;
                        if (!FountainFamily.objectRecycler.has(caloriesAsset)) FountainFamily.objectRecycler.putNew(caloriesAsset, new Image(TextBitmap.createTexture(caloriesText, FontLibrary.BOOSTER_NEXT_FY_BOLD, null, caloriesFontSize, NaN, caloriesColor, caloriesColor, 1, 1, -25, -25, TextSpriteAlign.RIGHT)));
                        imageTextCalories2[totalCupsLocale] = FountainFamily.objectRecycler.get(caloriesAsset);
                        caloriesSprite[u].addChild(imageTextCalories2[totalCupsLocale]);
                        totalCupsLocale ++;

                    }

                    //For the flavor calories copy
                    logoImageTextTitle = "FLAVOR_SHOTS_HAVE_ZERO_CALORIES" + "_" + u;
                    titleText = StringList.getList(FountainFamily.LOCALE_ISO[u]).getString("brand/calories-flavors");
                    if (!FountainFamily.objectRecycler.has(logoImageTextTitle)) FountainFamily.objectRecycler.putNew(logoImageTextTitle, new Image(TextBitmap.createTexture(titleText, FontLibrary.BOOSTER_FY_REGULAR, null, options.fontSizeFlavorCalories, NaN, colorTextNormal & 0xffffff, colorTextStrong & 0xffffff, 1, 1, -25, -25, TextSpriteAlign.LEFT, false, NaN, 0, null, 5)));
                    flavorsCaloriesCopy[u] = FountainFamily.objectRecycler.get(logoImageTextTitle);
                    flavorsCaloriesCopy[u].alpha = alphaText;

                    if(numMaxFlavors > 0  && !Calories.CALORIES_ON_FLAVOR) caloriesSprite[u].addChild(flavorsCaloriesCopy[u]);
                }

                //For the name of the beverage above the calories information
                if(Calories.DISPLAY_NAME) {
                    logoImageTextTitle = beverage.name + "_calories_" + String(__options.isADA);
                    if (!FountainFamily.objectRecycler.has(logoImageTextTitle)) FountainFamily.objectRecycler.putNew(logoImageTextTitle, new Image(TextBitmap.createTexture(beverage.name.toUpperCase(), FontLibrary.BOOSTER_NEXT_FY_BOLD, null, options.fontSizeFlavorCalories, NaN, caloriesColor, caloriesColor, 1, 1, -25, -25, TextSpriteAlign.RIGHT, false, NaN, 0, null, 7)));
                    beverageName = FountainFamily.objectRecycler.get(logoImageTextTitle);
                    addChild(beverageName);
                }

                caloriesSprite[currentLanguage].visible = true;
            }
		...
		private function redrawVisibility():void {
            ...
            if(Calories.CALORIES_ACTIVE) {
                ...
                caloriesSprite[currentLanguage].alpha = Equations.expoOut(uiPhase);

                if(Calories.DISPLAY_NAME) {
                    beverageName.x = width - beverageName.width - options.marginCaloriesRight;
                    beverageName.y = caloriesHeight - beverageName.height - 15;
                    beverageName.alpha = Equations.expoOut(uiPhase);
                }

                for(i = 0; i < totalCupsLocale; i ++) {
                    ...
                    u += 2;
                    if(u * 0.5 % Calories.CUPS.length == 0) {
                        u = 0;
                        separation = 0;
                    }
                    separation += 15;
                ...
                for(i = 0; i < FountainFamily.LOCALE_ISO.length; i ++) {
                    try {
                        flavorsCaloriesCopy[i].y = options.marginTitleFromTop + (imageTextTitleLine2[0] != null ? imageTextTitleLine2[0].height : imageTextTitleLine1[0].height) - 10;
                        flavorsCaloriesCopy[i].x = MathUtils.map(Equations.expoOut(uiPhase), 0, 1, -imageTextTitleLine1[0].width, options.marginTitleLeft);
                    } catch(e : Error) {
                        //Quick fix for the mixes.
                        flavorsCaloriesCopy[i].y = -1000;
                        flavorsCaloriesCopy[i].x = -1000;
                    }
                }
		...
		override public function dispose():void {
            ...
            if(Calories.CALORIES_ACTIVE) {
                ...
                if(Calories.DISPLAY_NAME) {
                    removeChild(beverageName);
                    FountainFamily.objectRecycler.putBack(beverageName);
                    beverageName = null;
                }

                //Calories copy for each cup
                for(var s : uint = 0; s < totalCupsLocale; s ++) {
                    imageTextCalories1[s].parent.removeChild(imageTextCalories1[s]);
                    FountainFamily.objectRecycler.putBack(imageTextCalories1[s]);
                    imageTextCalories1[s] = null;

                    imageTextCalories2[s].parent.removeChild(imageTextCalories2[s]);
                ...
                for(j = 0; j < FountainFamily.LOCALE_ISO.length; j ++) {
                    ...
                    if(!beverage.isMix) {
                        if (flavorsCaloriesCopy[j] != null) {
                            if(!Calories.CALORIES_ON_FLAVOR) flavorsCaloriesCopy[j].parent.removeChild(flavorsCaloriesCopy[j]);
                            FountainFamily.objectRecycler.putBack(flavorsCaloriesCopy[j]);
                            flavorsCaloriesCopy[j] = null;
                        }
                    }

                    removeChild(caloriesSprite[j]);
                    caloriesSprite[j].dispose();
                    caloriesSprite[j] = null;
                }
		...
		public function set language(value : uint) : void {
            ...
            if(Calories.CALORIES_ACTIVE) {
                for(i = 0; i < FountainFamily.LOCALE_ISO.length; i ++) {
                    if(caloriesSprite[i] != null) caloriesSprite[i].visible = false;
                }
            }
            if(caloriesSprite[currentLanguage] != null) caloriesSprite[currentLanguage].visible = true;
